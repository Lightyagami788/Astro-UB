...
async def upstream(ups):
    ...
    try:
        ...
        return 
          ...
          if conf != "now":
            await ups.edit(
                f"**👀You want to Update Your 👀Astro okok...Use → `{xxxx}update now`\n and update Your Astro If any Update available...."
            )
            return
    repo = Repo.init()
    ...
    origin.fetch()
    force_updateme = True
    repo.create_head("lover", origin.refs.lover)
    repo.heads.lover.set_tracking_branch(origin.refs.lover)
    repo.heads.lover.checkout(True)
    ...
    if Config.HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await ups.edit(
            "**Astro🌌Started to update....Just wait till New deploy sucessfully text in delivered in Your Private GROUP."
        )
        ups_rem.fetch(ac_br)
